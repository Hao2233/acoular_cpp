...
public:
    ...
    // 网格点总数。只读；当设置其他网格定义属性时自动设置
    virtual int size() const = 0;

    // 网格形状。只读，以元组的形式给出，对于笛卡尔网格很有用
    virtual std::tuple<int, int> shape() const = 0;

    // 网格位置，作为大小为(3, size)的浮点数数组，不包括无效的麦克风；只读。
